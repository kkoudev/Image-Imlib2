...
Image::Imlib2
Imlib2_load_scale(packname="Image::Imlib2", filename, scale_width, scale_height)
        char * packname
  char * filename
  int scale_width
  int scale_height
        ...
        CODE:
  {
    Imlib_Image image;
                ...
                image = imlib_load_image_with_error_return_and_scale_size (filename, scale_width, scale_height, &err);
                if (err == IMLIB_LOAD_ERROR_FILE_DOES_NOT_EXIST) {
                  Perl_croak(aTHX_ "Image::Imlib2 load error: File does not exist");
                }
